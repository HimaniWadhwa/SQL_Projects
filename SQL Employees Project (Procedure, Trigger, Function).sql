...
Select d.dept_name, e.gender, Avg(s.salary) From employees e Join Salaries s ON e.emp_no = s.emp_no 
JOIN dept_emp de on s.emp_no = de.emp_no Join departments d on d.dept_no = de.dept_no GROUP BY d.dept_name, e.gender ORDER BY d.dept_name;
...
# - employee number
# - the lowest department number among the departments where the employee has worked in (use a subquery to retrieve this value from the 'dept_emp' table)
...
SELECT 
    ...
    (SELECT 
        ...
        FROM
            dept_emp d
        WHERE
            e.emp_no = d.emp_no) Dept_no,
    CASE
        WHEN emp_no <= '10020' THEN '10022'
        ELSE '110039'
    END AS manager
FROM
    employees e
